...
- Windows 11, 64-bit, build 22000 or newer. Windows 10 and 32-bit Windows are not supported or certified.
...
| AMD | Windows discovery and ADLX capability probe | Driver-reported ADLX controls, fan curves and exact Variable Graphics Memory states only |
...
- Intel PresentMon 2.5.1-compatible installed runtime for vendor-neutral frame data; extended presentation-path, CPU/GPU wait, latency, tearing and PSO observations appear only when that runtime supplies them.
...
- AMD Variable Graphics Memory remains hidden unless ADLX reports exact supported states for the selected physical adapter; applying a state may require a restart.
- Per-game Compatibility Mode can disable tuning, OSD, PresentMon, hotkeys and capture for individual titles without changing the global capability contract.
...
Confirm the user has Windows 11 x64 build 22000 or newer, an official current GPU driver and a physical NVIDIA, AMD or Intel GPU. Never promise that every control exists on every GPU; vendor API, firmware, driver branch, notebook policy and OEM limits decide the actual writable set.
